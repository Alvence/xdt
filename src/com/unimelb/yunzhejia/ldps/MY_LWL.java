...
//	    }
	    ...
	    int c = 1;
	    double obj = getObj(m_Train, expls);
	    System.out.println(obj);
	    while(!converge){
	    	for(int i = 0; i < omega.length; i++){
//	    		System.out.println(Arrays.toString(omega[i]));
//	    		System.out.println(i);
	    		...
	    		if(expls!=null && expls.get(m_Train.get(i).getID())!=null){
	    			...
	    			double[] ex = new double[expl.size()];
	    			for(int j = 0;j < ex.length;j++){
	    				ex[j] = expl.get(j);
	    			}
	    			Utils.normalize(ex);
	    			for(int j = 0; j < ex.length;j++){
	    				ex[j] *=  (m_Train.numAttributes()-1);
		    		}
	    			for(int j = 0; j < e.length; j++){
	    				e[j] = beta*(omega[i][j]-ex[j]);
	    			}
	    		}
	    		for(int j = 0; j < e.length; j++){
...
//	    		System.out.println(Arrays.toString(gr));
	    		Utils.normalize(omega[i]);
	    		for(int j = 0; j < omega[i].length;j++){
	    			omega[i][j] *=  (m_Train.numAttributes()-1);
	    		}
	    	}
	    	double newObj = getObj(m_Train, expls);
	    	if(Math.abs(obj-newObj)<0.01){
	    		converge = true;
	    	}
	    	obj = newObj;
		    System.out.println((c++)+" "+newObj);
	    }
	    
	    updateCurrentTrain();   
	  }
  double getObj(Instances data, Map<Long, List<Integer>> expls) throws Exception{
	  MY_LWL temp = (MY_LWL)this.makeCopy(this);
	  temp.updateCurrentTrain();
	  Evaluation eval = new Evaluation(data);
		
	  eval.evaluateModel(temp, data);
	  double err = eval.pctIncorrect();
	  double errX = 0;
	  for(int i = 0; i < data.numInstances(); i++){
		  Instance ins = data.get(i);
		  if(expls!=null && expls.get(ins.getID())!=null){
  			List<Integer> expl = expls.get(ins.getID());
  			double[] ex = new double[expl.size()];
  			for(int j = 0;j < ex.length;j++){
  				ex[j] = expl.get(j);
  			}
  			Utils.normalize(ex, m_Train.numAttributes()-1);
  			for(int j = 0; j < ex.length; j++){
  				errX += 0.5*beta*(omega[i][j]-ex[j])*(omega[i][j]-ex[j]);
  			}
  		}
	  }
