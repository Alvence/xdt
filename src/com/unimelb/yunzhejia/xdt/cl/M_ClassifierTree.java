 ...
 * @version $Revision$
 */
public class M_ClassifierTree implements Drawable, Serializable,
  ...
  protected M_ModelSelection m_toSelectModel;

  /** Local model at node. */
  public M_ClassifierSplitModel m_localModel;

  /** References to sons. */
  public M_ClassifierTree[] m_sons;
  ...
  protected M_Distribution m_test;
   ...
   * Constructor.
   */
  public M_ClassifierTree(M_ModelSelection toSelectLocModel) {
  ...
  public void buildClassifierWithExpl(Instances data, Map<Long, Set<Integer>> expls) throws Exception {
	// can classifier tree handle the data?
	    ...
	    buildTreeWithExpl(data, false, expls);
  }
  ...
  public void buildTreeWithExpl(Instances data, boolean keepData, Map<Long, Set<Integer>> expls) throws Exception {
	    ...
	    m_isEmpty = false;
	    m_sons = null;
	    m_localModel = m_toSelectModel.selectModelWithExpl(data, expls);
	    if (m_localModel.numSubsets() > 1) {
	      localInstances = m_localModel.split(data);
	      data = null;
	      m_sons = new M_ClassifierTree[m_localModel.numSubsets()];
	      for (int i = 0; i < m_sons.length; i++) {
	        m_sons[i] = getNewTreeWithExpl(localInstances[i],expls);
  ...
  protected  M_ClassifierTree getNewTreeWithExpl(Instances instances, Map<Long, Set<Integer>> expls) throws Exception {
	  M_ClassifierTree newTree = new M_ClassifierTree(m_toSelectModel);
	    newTree.buildTreeWithExpl(instances, false, expls);
...
/**
  ...
  public final void cleanup(Instances justHeaderInfo) {
    ...
    m_test = null;
    if (!m_isLeaf) {
      for (M_ClassifierTree m_son : m_sons) {
  ...
  public StringBuffer[] toSource(String className) throws Exception {
    ...
    if (m_isLeaf) {
      ...
      long printID = M_ClassifierTree.nextID();
   ...
   * @throws Exception if something goes wrong
   */
  protected M_ClassifierTree getNewTree(Instances data) throws Exception {
   ...
   * @throws Exception if something goes wrong
   */
  protected M_ClassifierTree getNewTree(Instances train, Instances test)
    throws Exception {
   ...
   */
  private M_ClassifierSplitModel localModel() {
   ...
   * Method just exists to make program easier to read.
   */
  private M_ClassifierTree son(int index) {
  ...
  public double[] getMembershipValues(Instance instance) throws Exception {
    ...
    Queue<M_ClassifierTree> queueOfNodes = new LinkedList<M_ClassifierTree>();
    ...
    while (!queueOfNodes.isEmpty()) {
      ...
      M_ClassifierTree node = queueOfNodes.poll();
